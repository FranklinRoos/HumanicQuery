...
function functieQuery ()
{
  ...
  /*  if(!isSet($_SESSION['kopText']))
    ...
    $_SESSION['functie'] = 'Front-end developer';
    $_SESSION['regio'] = 'Amsterdam e.o.';
    $_SESSION['kopText'] = "Overzicht ".$_SESSION['functie']."s met voorkeur ".$_SESSION['regio']. "";
...
if(isSet($_SESSION['blad'])&& $_SESSION['blad'] !='user_blad2')    
{
  ...
  $_SESSION['user_page2']=0;// hierdoor kom ik in de eerste pagina van het kandidaat overzicht als ik uit de kandidaat.php kom na het bekijken van een profiel   
    ...
    $users = mysqli_query($connection, "SELECT DISTINCT `user_id`,`achternaam`, `tussenvoegsel`, `voornaam`, `foto`, `cv`, `ervaring` FROM `kandiaten_vw`  WHERE  `functie_naam` = '".$_SESSION['functie']."' AND  `regio_naam`  = '".$_SESSION['regio']. "'  ");
    ...
    $objecten = mysqli_query($connection, "SELECT DISTINCT `user_id`,`achternaam`, `tussenvoegsel`, `voornaam`, `foto`, `cv`, `ervaring` FROM `kandiaten_vw`  WHERE  `functie_naam` = '".$_SESSION['functie']."' AND  `regio_naam`  = '".$_SESSION['regio']. "'   LIMIT $start,10") or die(mysqli_error());//weergave per 10
    $count_Limit_users = mysqli_num_rows($objecten);//tellen hoeveel users er (nog) in de ($objecten) presentatie  zitten
  ...
  // vanaf hier naar beneden heb ik uitgeschakeld op zaterdag 20 aug
